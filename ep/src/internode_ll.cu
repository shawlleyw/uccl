...
__launch_bounds__(kNumThreads, 1) __global__
  ...
  // Cross-rank barrier BEFORE cleaning. Required because peers may still be
  // issuing IPC writes to our signaling buffer (e.g., last NORMAL-mode chunked
  // EP, or a lagging rank's first LL dispatch). Without this barrier, a
  // peer's late write can land *before* our clean and be wiped out, leaving
  // us spinning forever in the next dispatch_recv.
...
#pragma unroll
  ...
  // Cross-rank barrier AFTER cleaning. Required so that no peer enters its
  // first post-clean LL dispatch (and writes IPC counts to OUR signaling
  // buffer) until WE have finished cleaning. Otherwise a fast peer's
  // dispatch write to our slot is then zeroed by our slow clean, and we
  // hang on dispatch_recv forever.
  if constexpr (kNumRanks > 1) {
    barrier_block<kNumRanks>(barrier_signal_ptrs, rank);
  }
}
